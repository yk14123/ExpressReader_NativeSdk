package com.cnpeak.expressreader.lru;

import com.cnpeak.expressreader.global.Global;
...
public class LruMap {
    ...
    public <T> T createOrGetElement(String key, Class<T> cla,
                                    SoftMap.ICreateObjectAble<T> createObjectAble) {
        return createOrGetElement(key, cla, createObjectAble, false);
    }

    @SuppressWarnings({"unchecked", "WeakerAccess"})
    public <T> T createOrGetElement(String key, Class<T> cla,
                                    SoftMap.ICreateObjectAble<T> createObjectAble, boolean serialize) {
        T t = (T) get(key);
        if (t != null) {
